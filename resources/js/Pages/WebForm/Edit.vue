                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <form @submit.prevent="formUpdate">
                        <div class="p-6 bg-white border-b border-gray-200 text-right">
                            ...
                            <InputError v-if="form.errors" class="mt-2" :message="form.errors.locale"/>
                        ...
                        <div class="p-6 bg-white border-b border-gray-200">
                            <div class="mt-4">
                                ...
                                <textarea id="content" type="text" class="mt-1 block w-full" v-model="form.content"
                                          required></textarea>
                                <InputError v-if="form.errors" class="mt-2" :message="form.errors.content"/>
                            </div>
                        </div>
                        <div class="p-6 bg-white border-b border-gray-200">
                            <div class="mt-4">
                                <InputLabel for="Crm_source" value="Crm source"/>
                                <TextInput id="Crm_source" type="text" class="mt-1 block w-full" v-model="form.crm_source"></TextInput>
                                <InputError v-if="form.errors" class="mt-2" :message="form.errors.crm_source"/>
                            </div>
                        </div>
                        <div class="p-6 bg-white border-b border-gray-200">
                            <div class="mt-4">
                                ...
                                <InputError v-if="form.errors" class="mt-2" :message="form.errors.recaptcha_site_key"/>
                            </div>
                        </div>
                        <div class="p-6 bg-white border-b border-gray-200 text-right">
                            <primary-button :disabled="form.processing">Save</primary-button>
...
export default {
    ...
    methods: {
        formUpdate(){
            this.form.patch(this.route('webforms.update', {id: this.webform.id}))
        }
    },
    mounted() {
        this.form.locale = this.webform.locale;
        this.form.content = JSON.parse(this.webform.content);
        this.form.crm_source = this.webform.crm_source;
        this.form.recaptcha_site_key = this.webform.recaptcha_site_key;
    }
}
</script>
